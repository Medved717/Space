...
def titanic_avg(df):
    avg_pclass = df.groupby('Pclass')['Fare'].sum()
    sort_passenger_id_pclass = df.groupby('Pclass')['PassengerId'].count()
    sort_avg_pclass = avg_pclass / sort_passenger_id_pclass
    result_dict = sort_avg_pclass.to_dict()
    result_json = json.dumps(result_dict)
    return result_json

result_func = titanic_avg(df)
print(result_func)